...
class Pose2VideoLongSVDPipeline(DiffusionPipeline):
    ...
    def _clip_encode_image(self, image, audio_prompts, uncond_audio_prompts, vasa_prompts, uncond_vasa_prompts, num_frames, device, num_videos_per_prompt, do_classifier_free_guidance, frames_per_batch):
        ...
        if do_classifier_free_guidance:
            ...
            vasa_prompts = torch.cat([uncond_vasa_prompts, uncond_vasa_prompts, uncond_vasa_prompts, vasa_prompts])
            ...
            for i, t in enumerate(timesteps):
                ...
                for batch, index_start in enumerate(range(0, num_frames+frames_per_batch, frames_per_batch - overlap)):
                    ...
                    noise_pred = self.unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=(batch_image_embeddings.flatten(0,1), [batch_audio_prompts.flatten(0,1)*gate[0],batch_vasa_prompts.flatten(0,1)*gate[1]]),
                    ...
                    # perform guidance
                    if do_classifier_free_guidance:
                        noise_pred_uncond, noise_pred_drop_audio_vasa, noise_pred_drop_vasa, noise_pred_cond= noise_pred.chunk(4)
                        noise_pred = noise_pred_uncond + self.guidance_scale1[i] * (noise_pred_drop_audio_vasa - noise_pred_uncond) + self.guidance_scale2[i] * (noise_pred_drop_vasa - noise_pred_drop_audio_vasa) + self.guidance_scale3[i] * (noise_pred_cond - noise_pred_drop_vasa)
